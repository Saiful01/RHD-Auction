                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.bids.store") }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('bidder') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="bidder_id" id="bidder_id" required>
                                @foreach($bidders as $id => $entry)
                                    <option value="{{ $id }}" {{ old('bidder_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('auction') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="auction_id" id="auction_id" required>
                                @foreach($auctions as $id => $entry)
                                    <option value="{{ $id }}" {{ old('auction_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('vat') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="vat" id="vat" value="{{ old('vat', '') }}" step="0.0000001">
                            @if($errors->has('vat'))
                        ...
                        <div class="form-group {{ $errors->has('tax') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="tax" id="tax" value="{{ old('tax', '') }}" step="0.0000001">
                            @if($errors->has('tax'))
                        ...
                        <div class="form-group {{ $errors->has('bid_amount') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="bid_amount" id="bid_amount" value="{{ old('bid_amount', '') }}" step="0.0000001">
                        ...
                        <div class="form-group {{ $errors->has('total_amount') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="total_amount" id="total_amount" value="{{ old('total_amount', '') }}" step="0.0000001">
                        ...
                        <div class="form-group {{ $errors->has('is_condition_accept') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\Bid::IS_CONDITION_ACCEPT_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="is_condition_accept_{{ $key }}" name="is_condition_accept" value="{{ $key }}" {{ old('is_condition_accept', '0') === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('is_winner') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\Bid::IS_WINNER_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="is_winner_{{ $key }}" name="is_winner" value="{{ $key }}" {{ old('is_winner', '0') === (string) $key ? 'checked' : '' }}>
