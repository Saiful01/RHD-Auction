        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.lot-items.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('lot') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="lot_id" id="lot_id" required>
                                @foreach($lots as $id => $entry)
                                    <option value="{{ $id }}" {{ old('lot_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('lot'))
                        ...
                        <div class="form-group {{ $errors->has('tree_no') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="tree_no" id="tree_no">{!! old('tree_no') !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('dia') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="dia" id="dia" value="{{ old('dia', '') }}">
                            @if($errors->has('dia'))
                        ...
                        <div class="form-group {{ $errors->has('quantity') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="quantity" id="quantity" value="{{ old('quantity', '') }}" step="0.000000001">
                        ...
                        <div class="form-group {{ $errors->has('estimated_price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="estimated_price" id="estimated_price" value="{{ old('estimated_price', '') }}" step="0.000000001">
