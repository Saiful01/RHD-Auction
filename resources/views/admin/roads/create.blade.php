...
Dropzone.options.imageDropzone = {
    ...
    removedfile: function (file) {
        ...
        var name = typeof file.file_name !== 'undefined' ? file.file_name : uploadedImageMap[file.name]
        $('form').find('input[name="image[]"][value="' + name + '"]').remove()
    },
    init: function () {
@if(isset($road) && $road->image)
        var files = {!! json_encode($road->image) !!}
        for (var i in files) {
            var file = files[i]
...
Dropzone.options.filesDropzone = {
    ...
    removedfile: function (file) {
        file.previewElement.remove()
        var name = typeof file.file_name !== 'undefined' ? file.file_name : uploadedFilesMap[file.name]
        $('form').find('input[name="files[]"][value="' + name + '"]').remove()
    },
    init: function () {
@if(isset($road) && $road->files)
        var files = {!! json_encode($road->files) !!}
        for (var i in files) {
            var file = files[i]
            this.options.addedfile.call(this, file)
            file.previewElement.classList.add('dz-complete')
