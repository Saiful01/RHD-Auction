        ...
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.bidder-auction-requests.create') }}">
                    {{ trans('global.add') }} {{ trans('cruds.bidderAuctionRequest.title_singular') }}
                </a>
            </div>
        </div>
    @endcan
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-bidderBidderAuctionRequests">
                            <thead>
                                <tr>
                                    <th width="10">
                                    ...
                                    <tr data-entry-id="{{ $bidderAuctionRequest->id }}">
                                        ...
                                        <td>
                                            @can('bidder_auction_request_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('admin.bidder-auction-requests.show', $bidderAuctionRequest->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('bidder_auction_request_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('admin.bidder-auction-requests.edit', $bidderAuctionRequest->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('bidder_auction_request_delete')
                                                <form action="{{ route('admin.bidder-auction-requests.destroy', $bidderAuctionRequest->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-bidderBidderAuctionRequests:not(.ajaxTable)').DataTable({ buttons: dtButtons })
