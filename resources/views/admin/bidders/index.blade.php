                            ...
                            <tbody>
                                @foreach($bidders as $key => $bidder)
                                    <tr data-entry-id="{{ $bidder->id }}">
                                        ...
                                        <td>
                                            @can('bidder_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('admin.bidders.show', $bidder->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('bidder_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('admin.bidders.edit', $bidder->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('bidder_delete')
                                                <form action="{{ route('admin.bidders.destroy', $bidder->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-Bidder:not(.ajaxTable)').DataTable({ buttons: dtButtons })
