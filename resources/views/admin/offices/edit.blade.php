                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.offices.update", [$office->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('office_type') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="office_type_id" id="office_type_id" required>
                                @foreach($office_types as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('office_type_id') ? old('office_type_id') : $office->office_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('office_name_en') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="office_name_en" id="office_name_en" value="{{ old('office_name_en', $office->office_name_en) }}">
                        ...
                        <div class="form-group {{ $errors->has('office_name_bn') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="office_name_bn" id="office_name_bn" value="{{ old('office_name_bn', $office->office_name_bn) }}">
                        ...
                        <div class="form-group {{ $errors->has('office_cat') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\Office::OFFICE_CAT_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="office_cat_{{ $key }}" name="office_cat" value="{{ $key }}" {{ old('office_cat', $office->office_cat) === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('parent_office') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="parent_office" id="parent_office" value="{{ old('parent_office', $office->parent_office) }}" step="1">
