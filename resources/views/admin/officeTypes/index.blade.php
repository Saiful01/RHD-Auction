        ...
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.office-types.create') }}">
                    {{ trans('global.add') }} {{ trans('cruds.officeType.title_singular') }}
                </a>
            </div>
        </div>
    @endcan
                        ...
                        <table class=" table table-bordered table-striped table-hover datatable datatable-OfficeType">
                            <thead>
                                <tr>
                                    ...
                                    <th>
                                        {{ trans('cruds.officeType.fields.id') }}
                                    </th>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $officeType->id }}">
                                        ...
                                        <td>
                                            @can('office_type_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('admin.office-types.show', $officeType->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('office_type_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('admin.office-types.edit', $officeType->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('office_type_delete')
                                                <form action="{{ route('admin.office-types.destroy', $officeType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-OfficeType:not(.ajaxTable)').DataTable({ buttons: dtButtons })
